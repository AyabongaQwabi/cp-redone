...
export default function ManageDepartmentEmployeesPage({
  params,
}: {
  params: { id: string };
}) {
    ...
    async function fetchData() {
      try {
        ...
        const assignedEmployeesData = employeesData
          .filter((employee) =>
            assignments.some((a) => a.employeeId === employee.id)
          )
          .map((employee) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const currentlyAssignedIds = assignedEmployees.map(
      ...
      const employeesToAdd = Object.entries(selectedEmployees)
        .filter(
          ([id, isSelected]) => isSelected && !currentlyAssignedIds.includes(id)
        )
        .map(([id]) => id);
      ...
      const employeesToUpdate = assignedEmployees
        .filter(
          (employee) =>
            ...
            (employee.role !== employeeRoles[employee.id] ||
              employee.isManager !== employeeManagers[employee.id])
        )
        .map((employee) => ({
      ...
      <div className='flex items-center mb-6'>
        ...
        <div className='flex-1'>
          ...
          <p className='text-gray-600'>
            Assign or remove employees for{' '}
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                    ...
                    <TableRow key={employee.id}>
                      ...
                      <TableCell className='text-center'>
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handleManagerChange(employee.id, !!checked)
                          }
                          disabled={!selectedEmployees[employee.id]}
                        />
                      </TableCell>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Employees from this company that can be assigned to the department
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {filteredAvailableEmployees.length > 0 ? (
              ...
              <div className='text-center py-8'>
                {employees.length === 0 ? (
                  ...
                  <p className='text-gray-500'>
                    All employees are already assigned to this department
                  </p>
                )}
              </div>
            )}
